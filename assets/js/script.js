...
var taskFormHandler = function (event) {
    ...
    // reset form fields for next track to be entered
    document.querySelector("input[name='task-name']").value = "";
    document.querySelector("select[name='task-type']").selectedIndex = 0;

    var isEdit = formEl.hasAttribute("data-task-id");

    //data has attribure so get task id call function to complete edit
    if (isEdit) {
        var taskId = formEl.getAttribute("data-task-id");
        completeEditTask(taskNameInput, taskTypeInput, taskId);
    }

    //no data attribute create normal Process
    else {
        var taskDataObj = {
            name: taskNameInput,
            type: taskTypeInput,
...
var createTaskEl = function (taskDataObj) {
    ...
    switch (taskDataObj.status) {
        ...
        case "completed":
            ...
            taskCompletedEl.append(listItemEl);
            break;
        default:
...
var taskStatusChangeHandler = function (event) {
    ...
    //get the currently selected options value and convert to lc
